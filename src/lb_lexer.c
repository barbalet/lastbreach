...
void lx_next_token(Lexer *lx) {
  ...
  switch (c) {
    case '{': lx->cur = tk_make(lx, TK_LBRACE, s, 1);
    return;
    case '}': lx->cur = tk_make(lx, TK_RBRACE, s, 1);
    return;
    case '(': lx->cur = tk_make(lx, TK_LPAREN, s, 1);
    return;
    case ')': lx->cur = tk_make(lx, TK_RPAREN, s, 1);
    return;
    case '[': lx->cur = tk_make(lx, TK_LBRACK, s, 1);
    return;
    case ']': lx->cur = tk_make(lx, TK_RBRACK, s, 1);
    return;
    case ':': lx->cur = tk_make(lx, TK_COLON, s, 1);
    return;
    case ';': lx->cur = tk_make(lx, TK_SEMI, s, 1);
    return;
    case ',': lx->cur = tk_make(lx, TK_COMMA, s, 1);
    return;
    case '.': if (lx_match(lx, '.')) {
    ...
    return;
    case '+': lx->cur = tk_make(lx, TK_PLUS, s, 1);
    return;
    case '-': lx->cur = tk_make(lx, TK_MINUS, s, 1);
    return;
    case '*': lx->cur = tk_make(lx, TK_STAR, s, 1);
    return;
    case '/': lx->cur = tk_make(lx, TK_SLASH, s, 1);
    return;
    case '=': if (lx_match(lx, '=')) {
